    ...
    <form @submit.prevent="submit">
        <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
            ...
            <div class="mt-10 grid grid-cols-1 gap-6 max-md:grid-cols-1">
                ...
                <div>
                    <span class="p-float-label">
                        <Textarea v-model="form.descricao" :autoResize="true" rows="3" cols="60" class="w-full" required />
                        <label for="descricao" class="text-sm">Descrição</label>
                    </span>
                </div>
                <div>
                    <span class="p-float-label">
                        <InputText v-model="form.valor" id="valor" type="number" class="w-full" required maxlength="50" />
...
const form = useForm({
    ...
    nome: $propsPage?.value.ConfigServicos?.nome,

    descricao: $propsPage?.value.ConfigServicos?.descricao,

    valor: $propsPage?.value.ConfigServicos?.valor,
