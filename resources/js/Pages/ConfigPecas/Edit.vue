    ...
    <form @submit.prevent="submit">
        <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
            <div class="flex flex-col space-y-1">
                ...
                <SectionTitle class="text-xs text-gray-600">Informações essenciais para edição de peças no sistema.
                </SectionTitle>
            </div>
...
const form = useForm({
    token: $propsPage?.value.ConfigPecas?.token,

    nome: $propsPage?.value.ConfigPecas?.nome,
...
function submit() {
    ...
    submitForm.post(
        route("update.ConfigPecas", { id: $propsPage.value?.ConfigPecas?.token }),
        {
            preserveState: true,
            onError: (errors) => {
