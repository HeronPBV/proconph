    ...
    <form @submit.prevent="submit">
        <section class="mt-6 bg-white rounded-sm p-10 shadow-sm" v-if="currentNav == 1">
            <div class="flex flex-col space-y-1">
                ...
                <SectionTitle class="text-xs text-gray-600">Informações essenciais para edição de Fornecedores no sistema.
                </SectionTitle>
            </div>
            ...
            <div class="flex space-x-5 mt-8">
                ...
                <Link :href="route('list.ConfigFornecedores')" as="button" type="button"
...
const form = useForm({
    token: $propsPage?.value.ConfigFornecedores?.token,

    nome: $propsPage?.value.ConfigFornecedores?.nome,

    endereco: $propsPage?.value.ConfigFornecedores?.endereco,

    telefone: $propsPage?.value.ConfigFornecedores?.telefone,

    email: $propsPage?.value.ConfigFornecedores?.email,

    status: { value: $propsPage?.value.ConfigFornecedores?.status },

    created_at: $propsPage?.value.ConfigFornecedores?.created_at,
});
...
function submit() {
    ...
    submitForm.post(
        route("update.ConfigFornecedores", { id: $propsPage.value?.ConfigFornecedores?.token }),
        {
            preserveState: true,
            onError: (errors) => {
