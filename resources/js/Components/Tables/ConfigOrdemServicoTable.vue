<template>
    <div class="bg-white px-4 shadow-lg rounded-xl">
        <p><br /></p>
        <AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.ConfigOrdemServico')"
            :RotaExcel="route('get.Excel.ConfigOrdemServico')" />
        <div class="flex justify-center items-center space-x-6 py-2">
            <!-- primeiro Card -->
            <div class="bg-white rounded-3xl shadow-lg w-96 max-h-[250px] border-card">
                ...
                <div class="flex ml-4 grid grid-cols-8 gap-8 max-md:grid-cols-1">
                    ...
                    <div>
                        <span class="p-float-label">
                            <Dropdown class="w-full" v-model="form2.id_veiculo" :options="veiculos"
                                optionLabel="name" dataKey="value" required filter="true" />
                            <label for="status" class="text-sm">Veiculos</label>
                        </span>
                    </div>
                    <div>
                        <span class="p-float-label">
                    ...
                    <div>
                        <span class="p-float-label">
                            <InputText v-model="form2.situacao" id="situacao" type="text" class="w-full" required />
                    ...
                    <div>
                        <span class="p-float-label">
                            <Checkbox inputId="binary" v-model="form2.limparFiltros" :binary="true"
                                style="margin: 2%" />
                ...
                <div class="space-x-6 flex ml-4 grid grid-cols-6 gap-6 max-md:grid-cols-1">
                    ...
                    <div class="flex items-center space-x-2">
                        <Checkbox @change="toggleColumns" :binary="true"
                            v-model="formColumns['columns']['id_veiculo']" />
                    ...
                    <div class="flex items-center space-x-2">
                        <Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['situacao']" />
                        <span>Situação</span>
                    </div>
                    <div class="flex items-center space-x-2">
        ...
        <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
            <div class="inline-block min-w-full py-2 align-middle">
                <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Ordens de serviço</b></h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                        <thead class="mb-24">
                            <tr class="text-gray-500 font-bold select-none" @click="setParams">
                                ...
                                <th v-if="formColumns.columns.id_veiculo" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group" @click="
                                        orderBy = {
                                            column: 'id_veiculo',
                                            sorting: sortTable(sortVal.id_veiculo)
                                                ? (sortVal.id_veiculo = 1)
                                                : (sortVal.id_veiculo = 0),
                                        }
                                        ">
                                    <div class="flex">
                                        <span class="group-hover:text-indigo-800">Veículo</span>
                                ...
                                <th v-if="formColumns.columns.descricao" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group" @click="
                                        orderBy = {
                                            column: 'descricao',
                                            sorting: sortTable(sortVal.descricao)
                                                ? (sortVal.descricao = 1)
                                                : (sortVal.descricao = 0),
                                        }
                                        ">
                                ...
                                <th v-if="formColumns.columns.situacao" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group" @click="
                                        orderBy = {
                                            column: 'situacao',
                                            sorting: sortTable(sortVal.situacao)
                                                ? (sortVal.situacao = 1)
                                                : (sortVal.situacao = 0),
                                        }
                                        ">
                                    <div class="flex">
                                        <span class="group-hover:text-indigo-800">Situação</span>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%" v-if="
                                    $page.props.userPermissions.includes('edit.ConfigOrdemServico') ||
                                    $page.props.userPermissions.includes('delete.ConfigOrdemServico')
                                ">
                                    Ações
                                </th>
                            </tr>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
                                ...
                                <td v-if="formColumns?.columns?.id_veiculo"
                                ...
                                <td v-if="formColumns?.columns?.proprietario"
                                    ...
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">
                                                {{ data?.veiculo.proprietario.nome }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td v-if="formColumns?.columns?.situacao"
                                    class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">{{ data?.situacao }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
                                    style="width: 5%" v-if="
                                        ...
                                        $page.props.userPermissions.includes('delete.ConfigOrdemServico')
                                    ">
                                    <Actions :routeDel="route('delete.ConfigOrdemServico', { id: data?.token })"
                                        :routeUpdate="route('form.update.ConfigOrdemServico', { id: data?.token })" />
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
console.log($propsPage?.value?.ConfigOrdemServico);
...
const formColumns = useForm({
    route_of_list: "list.ConfigOrdemServico",
    columns: {
        id_veiculo: validateColumnsVisibility("id_veiculo"),
        proprietario: validateColumnsVisibility("proprietario"),
        descricao: validateColumnsVisibility("descricao"),
        situacao: validateColumnsVisibility("situacao"),
...
const form2 = useForm({
    descricao: props.Filtros?.placa || null,
    situacao: props.Filtros?.modelo || null,
...
function del() {
    ...
    selected.value = "";
    Inertia.post(route("deleteSelected.ConfigOrdemServico", { id: valor.value }));
}
function delTodos() {
    Inertia.post(route("deletarTodos.ConfigOrdemServico"));
}

function RestaurarTodos() {
    Inertia.post(route("RestaurarTodos.ConfigOrdemServico"));
}
...
function FiltroAvancadoAplica() {
    ...
    !orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";
    submitForm.post(route("listP.ConfigOrdemServico"), {
        replace: false,
        data,
        onSuccess: () => {
