    ...
    <div class="bg-white px-4 shadow-lg rounded-xl">
        ...
        <SlideUpDown v-model="FiltroAvancado" :duration="300">
            <form @submit.prevent="submit">
                <div class="flex ml-4 grid grid-cols-8 gap-8 max-md:grid-cols-1">
                    ...
                    <div>
                        <span class="p-float-label">
                            <InputText v-model="form2.valor" id="valor" type="text" class="w-full" required />
        ...
        <SlideUpDown v-model="toggleFilter" :duration="300">
            ...
            <div class="text-sm text-gray-500 flex items-center">
                <div class="space-x-6 flex ml-4 grid grid-cols-6 gap-6 max-md:grid-cols-1">
                    ...
                    <div class="flex items-center space-x-2">
                        <Checkbox @change="toggleColumns" :binary="true" v-model="formColumns['columns']['valor']" />
                        <span>Valor</span>
                    </div>
                    <div class="flex items-center space-x-2">
        ...
        <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
            <div class="inline-block min-w-full py-2 align-middle">
                <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Servições</b></h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                        <thead class="mb-24">
                            <tr class="text-gray-500 font-bold select-none" @click="setParams">
                                ...
                                <th v-if="formColumns.columns.valor" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group" @click="
                                        orderBy = {
                                            column: 'valor',
                                            sorting: sortTable(sortVal.valor)
                                                ? (sortVal.valor = 1)
                                                : (sortVal.valor = 0),
                                        }
                                        ">
                                    <div class="flex">
                                        <span class="group-hover:text-indigo-800">Valor</span>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%" v-if="
                                    $page.props.userPermissions.includes('edit.ConfigServicos') ||
                                    $page.props.userPermissions.includes('delete.ConfigServicos')
                                ">
                                    Ações
                                </th>
                            </tr>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
                                ...
                                <td v-if="formColumns?.columns?.valor"
                                    class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">R$ {{ data?.valor.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</div>
                                        </div>
                                    </div>
                                </td>
...
const form2 = useForm({
    ...
    valor: props.Filtros?.valor || null,
