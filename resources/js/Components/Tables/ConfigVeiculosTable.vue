<template>
    <div class="bg-white px-4 shadow-lg rounded-xl">
        <p><br /></p>
        <AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.ConfigVeiculos')"
            :RotaExcel="route('get.Excel.ConfigVeiculos')" />
                ...
                <div class="flex ml-4 grid grid-cols-8 gap-8 max-md:grid-cols-1">
                    ...
                    <div>
                        <span class="p-float-label">
                            <Dropdown class="w-full" v-model="form2.id_tipo_veiculo" :options="tiposVeiculos"
                                optionLabel="name" dataKey="value" required filter="true" />
                            <label for="status" class="text-sm">Tipo de veículo</label>
                        </span>
                    </div>
                    <div>
                        <span class="p-float-label">
        ...
        <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
            <div class="inline-block min-w-full py-2 align-middle">
                <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Veiculos</b></h2>
                            ...
                            <tr class="text-gray-500 font-bold select-none" @click="setParams">
                                ...
                                <th v-if="formColumns.columns.id_tipo_veiculo" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group" @click="
                                        ...
                                        ">
                                    <div class="flex">
                                        <span class="group-hover:text-indigo-800">Tipo de veículo</span>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%" v-if="
                                    $page.props.userPermissions.includes('edit.ConfigVeiculos') ||
                                    $page.props.userPermissions.includes('delete.ConfigVeiculos')
                                ">
                                    Ações
                                </th>
                            </tr>
                            ...
                            <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
                                    ...
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">{{
                                                getTipoVeiculo(data?.id_tipo_veiculo) }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
                                    style="width: 5%" v-if="
                                        ...
                                        $page.props.userPermissions.includes('delete.ConfigVeiculos')
                                    ">
                                    <Actions :routeDel="route('delete.ConfigVeiculos', { id: data?.token })"
                                        :routeUpdate="route('form.update.ConfigVeiculos', { id: data?.token })" />
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
const getTipoVeiculo = (id) => {
    const tipo = tiposVeiculos.find(t => t.value === id);
...
const form2 = useForm({
    ...
    id_tipo_veiculo: props.Filtros?.id_tipo_veiculo || null,
...
function del() {
    ...
    selected.value = "";
    Inertia.post(route("deleteSelected.ConfigVeiculos", { id: valor.value }));
}
function delTodos() {
    Inertia.post(route("deletarTodos.ConfigVeiculos"));
}

function RestaurarTodos() {
    Inertia.post(route("RestaurarTodos.ConfigVeiculos"));
}
...
function FiltroAvancadoAplica() {
    ...
    !orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";
    submitForm.post(route("listP.ConfigVeiculos"), {
        replace: false,
        data,
        onSuccess: () => {
