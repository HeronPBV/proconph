<template>
    <div class="bg-white px-4 shadow-lg rounded-xl">
        <p><br /></p>
        <AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.ConfigPecas')"
            :RotaExcel="route('get.Excel.ConfigPecas')" />
        ...
        <SlideUpDown v-model="FiltroAvancado" :duration="300">
            <form @submit.prevent="submit">
                <div class="flex ml-4 grid grid-cols-8 gap-8 max-md:grid-cols-1">
                    ...
                    <div>
                        <span class="p-float-label">
                            <InputText v-model="form2.descricao" id="endereco" type="text" class="w-full" required />
                            <label for="endereco" class="text-sm">Descição</label>
                        </span>
                    </div>
                    <div>
                        <span class="p-float-label">
        ...
        <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
            <div class="inline-block min-w-full py-2 align-middle">
                <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Peças</b></h2>
                        ...
                        <thead class="mb-24">
                            <tr class="text-gray-500 font-bold select-none" @click="setParams">
                                ...
                                <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%" v-if="
                                    $page.props.userPermissions.includes('edit.ConfigPecas') ||
                                    $page.props.userPermissions.includes('delete.ConfigPecas')
                                ">
                                    Ações
                                </th>
                            </tr>
                            ...
                            <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
                                ...
                                <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
                                    style="width: 5%" v-if="
                                        ...
                                        $page.props.userPermissions.includes('delete.ConfigPecas')
                                    ">
                                    <Actions :routeDel="route('delete.ConfigPecas', { id: data?.token })"
                                        :routeUpdate="route('form.update.ConfigPecas', { id: data?.token })" />
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
const formColumns = useForm({
    route_of_list: "list.ConfigPecas",
    columns: {
...
function del() {
    ...
    selected.value = "";
    Inertia.post(route("deleteSelected.ConfigPecas", { id: valor.value }));
}
function delTodos() {
    Inertia.post(route("deletarTodos.ConfigPecas"));
}

function RestaurarTodos() {
    Inertia.post(route("RestaurarTodos.ConfigPecas"));
}
...
function FiltroAvancadoAplica() {
    ...
    !orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";
    submitForm.post(route("listP.ConfigPecas"), {
        replace: false,
        data,
        onSuccess: () => {
