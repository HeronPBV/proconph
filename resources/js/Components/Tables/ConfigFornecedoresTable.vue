<template>
    <div class="bg-white px-4 shadow-lg rounded-xl">
        <p><br /></p>
        <AchievementCard :Registros="props.Registros" :RotaCadastro="route('form.store.ConfigFornecedores')"
            :RotaExcel="route('get.Excel.ConfigFornecedores')" />
        ...
        <div class="mt-4 flex flex-col max-md:px-2 py-1 rounded-lg shadow-sm">
            <div class="inline-block min-w-full py-2 align-middle">
                <h2 class="text-xl font-semibold fontInter text-center py-5"><b>Fornecedores</b></h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                            ...
                            <tr class="text-gray-500 font-bold select-none" @click="setParams">
                                ...
                                <th v-if="formColumns.columns.endereco" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group" @click="
                                        orderBy = {
                                            column: 'endereco',
                                            sorting: sortTable(sortVal.endereco)
                                                ? (sortVal.endereco = 1)
                                                : (sortVal.endereco = 0),
                                        }
                                        ">
                                ...
                                <th v-if="formColumns.columns.telefone" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group" @click="
                                        orderBy = {
                                            column: 'telefone',
                                            sorting: sortTable(sortVal.telefone)
                                                ? (sortVal.telefone = 1)
                                                : (sortVal.telefone = 0),
                                        }
                                        ">
                                ...
                                <th v-if="formColumns.columns.email" scope="col"
                                    class="px-4 text-sm cursor-pointer text-center border-r group" @click="
                                        orderBy = {
                                            column: 'email',
                                            sorting: sortTable(sortVal.email)
                                                ? (sortVal.email = 1)
                                                : (sortVal.email = 0),
                                        }
                                        ">
                                    <div class="flex">
                                        <span class="group-hover:text-indigo-800">E-mail</span>
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="h-5 w-5 ml-auto group-hover:text-indigo-800" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <th scope="col" class="px-3 py-3.5 text-center text-sm sm:pr-12" style="width: 5%" v-if="
                                    $page.props.userPermissions.includes('edit.ConfigFornecedores') ||
                                    $page.props.userPermissions.includes('delete.ConfigFornecedores')
                                ">
                                    Ações
                                </th>
                            </tr>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr v-for="(data, key) in dataTable?.data" :key="key" class="hover:bg-indigo-50/20"
                                ...
                                <td v-if="formColumns?.columns?.endereco"
                                    class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                    <div class="flex items-center">
                                        <div>
                                ...
                                <td v-if="formColumns?.columns?.telefone"
                                    class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                    <div class="flex items-center">
                                        <div>
                                            <div class="font-medium text-gray-900">{{ data?.telefone }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td v-if="formColumns?.columns?.email"
                                    class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
                                    <div class="flex items-center">
                                        <div>
                                ...
                                <td class="whitespace-nowrap py-4 pl-3 pr-4 text-center text-sm font-medium sm:pr-6"
                                    style="width: 5%" v-if="
                                        ...
                                        $page.props.userPermissions.includes('delete.ConfigFornecedores')
                                    ">
                                    <Actions :routeDel="route('delete.ConfigFornecedores', { id: data?.token })"
                                        :routeUpdate="route('form.update.ConfigFornecedores', { id: data?.token })" />
                                </td>
                            </tr>
                        </tbody>
                    </table>
...
const formColumns = useForm({
    route_of_list: "list.ConfigFornecedores",
    columns: {
...
const form2 = useForm({
    ...
    endereco: props.Filtros?.endereco || null,
...
function del() {
    ...
    selected.value = "";
    Inertia.post(route("deleteSelected.ConfigFornecedores", { id: valor.value }));
}
function delTodos() {
    Inertia.post(route("deletarTodos.ConfigFornecedores"));
}

function RestaurarTodos() {
    Inertia.post(route("RestaurarTodos.ConfigFornecedores"));
}
...
function FiltroAvancadoAplica() {
    ...
    !orderBy.value?.length ? (data.orderBy = orderBy?.value) : "";
    submitForm.post(route("listP.ConfigFornecedores"), {
        replace: false,
        data,
        onSuccess: () => {
