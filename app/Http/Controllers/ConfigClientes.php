...
class ConfigClientes extends Controller
{
    public function index(Request $request)
    {
        ...
        try{
            ...
            $ConfigClientes = Cliente::withCount('veiculos')
                ->selectRaw("config_clientes.*, DATE_FORMAT(config_clientes.created_at, '%d/%m/%Y - %H:%i:%s') as data_final")
                ->where('config_clientes.deleted', '0');


            $sortableColumns = ['nome', 'cpf', 'email', 'telefone', 'endereco', 'status', 'created_at'];
            ...
            foreach ($filterableColumns as $column) {
                if (isset($data["ConfigClientes"][$column])) {
                    $AplicaFiltro = $data["ConfigClientes"][$column];
                    $ConfigClientes = $ConfigClientes->where("config_clientes.$column", "like", "%" . $AplicaFiltro . "%");
                }
            }
